...
*   **未连接 / 搜索状态**:
    *   **短按 [A]**: 扫描周边的富士相机 Wi-Fi AP，并自动发起连接与 7 步 PTP 握手。
    *   **短按 [B]**: 重置网络与连接状态。
*   **LiveView 实时取景模式 (连接成功后默认自动进入)** 📺:
    *   **短按 [A]**: 在取景画面下直接**触发快门拍照 (Shutter)** 📸。
    *   **短按 [B]**: 呼出**参数菜单仪表盘** 🎛️。
    *   **双击 [B]**: 切换**水平镜像翻转**（自拍取景神器）。
*   **参数菜单仪表盘 (Dashboard 菜单选择模式)** 🎛️:
    *   **短按 [B]**: 循环切换选中的参数卡片 (`ISO` -> `光圈` -> `快门` -> `EV`)。
    *   **IMU 体感上下左右倾斜**: 依据重力感应手势在 2×2 网格卡片中自如切换选中项。
    *   **短按 [A]**: 进入当前选中参数的 **IMU 体感滑条调参模式**。
    *   **长按 [B] (0.5s) / 双击 [B]**: 关闭菜单，返回 **LiveView 实时取景模式** 📺。
*   **IMU 体感调参模式 (Slider Adjust)**:
    *   **左右倾斜设备**: 依靠重力加速度计平滑连续增减参数档位。
    *   **短按 [A]**: 确认设定并同步至相机，返回参数菜单。
    *   **短按 [B]**: 取消修改，返回参数菜单。
